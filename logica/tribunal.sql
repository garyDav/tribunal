...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 13-05-2017 a las 00:58:41
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `pInsertComment`(IN `v_id_publication` INT, IN `v_id_user` INT, IN `v_description` TEXT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `pInsertCommunicate`(IN `v_id_use` INT, IN `v_id_usr` INT, IN `v_message` TEXT, IN `v_fec` DATETIME)
BEGIN
INSERT INTO communicate VALUES(null,v_id_use,v_id_usr,v_message,CURRENT_TIMESTAMP);
SELECT @@identity AS id, 'not' AS error;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `pInsertDepartment`(IN `v_name` VARCHAR(100), IN `v_cod_dep` VARCHAR(7))
BEGIN
INSERT INTO department VALUES(null,v_name,v_cod_dep);
SELECT @@identity AS id, 'not' AS error;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `pInsertJAgroambiental`(IN `v_id_municipality` INT, IN `v_name` VARCHAR(100), IN `v_cod_ja` VARCHAR(7))
BEGIN
INSERT INTO j_agroambiental VALUES(null,v_id_municipality,v_name,v_cod_ja);
SELECT @@identity AS id, 'not' AS error;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `pInsertMunicipality`(IN `v_id_province` INT, IN `v_name` VARCHAR(100))
BEGIN
INSERT INTO municipality VALUES(null,v_id_province,v_name);
SELECT @@identity AS id, 'not' AS error;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `pInsertPerson`(IN `v_ci` INT, IN `v_ex` VARCHAR(3), IN `v_name` VARCHAR(50), IN `v_last_name` VARCHAR(50), IN `v_fec_nac` DATE, IN `v_sex` VARCHAR(10))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `pInsertProvince`(IN `v_id_department` INT, IN `v_name` VARCHAR(100), IN `v_cod_dep` VARCHAR(7))
BEGIN
INSERT INTO province VALUES(null,v_id_department,v_name,v_cod_dep);
SELECT @@identity AS id, 'not' AS error;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `pInsertPublication`(IN `v_id_user` INT, IN `v_title` VARCHAR(255), IN `v_description` TEXT, IN `v_img` VARCHAR(255), IN `v_cod` VARCHAR(7))
BEGIN
INSERT INTO publication VALUES(null,v_id_user,v_title,v_description,CURRENT_TIMESTAMP,v_img,v_cod);
SELECT @@identity AS id, 'not' AS error;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `pInsertUser`(IN `v_id_person` INT, IN `v_email` VARCHAR(100), IN `v_pwd` VARCHAR(100), IN `v_type` VARCHAR(5), IN `v_cellphone` INT, IN `v_cod_dep` VARCHAR(7), IN `v_cod_ja` VARCHAR(7), IN `v_cod_all` VARCHAR(7))
BEGIN
...
SELECT @@identity AS id,v_type AS tipo, 'not' AS error;
ELSE
SELECT 'Error: Correo ya registrado.' error;
END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `pSession`(IN `v_email` VARCHAR(100), IN `v_pwd` VARCHAR(100))
BEGIN
DECLARE us int(11);
SET us = (SELECT id FROM user WHERE email LIKE v_email);
IF(us) THEN
IF EXISTS(SELECT id FROM user WHERE id = us AND pwd LIKE v_pwd) THEN
SELECT id,'not' error,type FROM user WHERE id = us;
ELSE
...
CREATE TABLE IF NOT EXISTS `j_agroambiental` (
  `id` int(11) NOT NULL,
...
INSERT INTO `province` (`id`, `id_department`, `name`, `cod_dep`) VALUES
(1, 1, 'Murillo', 'D-0001'),
(2, 2, 'Cercado', 'D-0002'),
(3, 3, 'Tomas Frias', 'D-0003'),
(4, 4, 'Cercado', 'D-0004'),
(5, 5, 'Oropeza', 'D-0005'),
(6, 6, 'Cercado', 'D-0006'),
(7, 7, 'Nicolas Suarez', 'D-0007'),
(8, 8, 'Cercado', 'D-0008'),
(9, 9, 'Andres de Ibañez', 'D-0009');
...
INSERT INTO `publication` (`id`, `id_user`, `title`, `description`, `fec`, `img`, `cod`) VALUES
...
INSERT INTO `user` (`id`, `id_person`, `email`, `pwd`, `type`, `cellphone`, `last_connection`, `registered`, `cod_dep`, `cod_ja`, `cod_all`) VALUES
(1, 1, 'juan@gmail.com', '585f7f3723df82f91fffd25a5c6900597cd4d1c1', 'supad', 88888888, '2017-05-07 00:38:31', '2017-05-07', 'D-0001', 'JA-0005', 'T-0000'),
(2, 2, 'leo@hotmail.com', '585f7f3723df82f91fffd25a5c6900597cd4d1c1', 'user', 72869852, '2017-05-11 09:17:28', '2017-05-11', 'D-0009', 'JA-0009', 'T-0000'),
(3, 8, 'jt@hotmail.com', '585f7f3723df82f91fffd25a5c6900597cd4d1c1', 'admin', 11111111, '2017-05-11 09:27:34', '2017-05-11', 'D-0002', 'JA-0004', 'T-0000'),
(5, 6, 'so@gmail.com', '585f7f3723df82f91fffd25a5c6900597cd4d1c1', 'admin', 74895632, '2017-05-11 09:31:56', '2017-05-11', 'D-0004', 'JA-0005', 'T-0000'),
(6, 7, 'vb@gmail.com', '585f7f3723df82f91fffd25a5c6900597cd4d1c1', 'admin', 74859632, '2017-05-11 09:54:26', '2017-05-11', 'D-0006', 'JA-0003', 'T-0000'),
(7, 13, 'fg@gmail.com', '585f7f3723df82f91fffd25a5c6900597cd4d1c1', 'admin', 74965823, '2017-05-11 09:56:08', '2017-05-11', 'D-0002', 'JA-0004', 'T-0000'),
(8, 3, 'lp@gmail.com', '585f7f3723df82f91fffd25a5c6900597cd4d1c1', 'user', 4579562, '2017-05-12 11:46:42', '2017-05-12', 'D-0003', 'JA-0001', 'T-0000');
...
--
ALTER TABLE `comment`
  ADD PRIMARY KEY (`id`), ADD KEY `id_publication` (`id_publication`), ADD KEY `id_user` (`id_user`);

--
-- Indices de la tabla `communicate`
--
ALTER TABLE `communicate`
  ADD PRIMARY KEY (`id`), ADD KEY `id_use` (`id_use`), ADD KEY `id_usr` (`id_usr`);
...
ALTER TABLE `j_agroambiental`
  ADD PRIMARY KEY (`id`), ADD KEY `id_municipality` (`id_municipality`);
...
ALTER TABLE `province`
  ADD PRIMARY KEY (`id`), ADD KEY `id_department` (`id_department`);
...
--
ALTER TABLE `user`
  ADD PRIMARY KEY (`id`), ADD KEY `id_person` (`id_person`);
